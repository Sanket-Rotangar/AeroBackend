...
class MQTTService {
  ...
  async handleLoRaGatewayData(payload) {
    ...
    const mac = node_id;
    const dataType = type || 'air_quality'; // Default to air_quality if type not specified
    ...
    try {
      ...
      const logMessage = dataType === 'wildlife' 
        ? `Wildlife detection data from node ${node_id} via ${gateway_id}`
        : `Air quality data from node ${node_id} via ${gateway_id}`;
